...
@testit.className('Материал')
@testit.description('Создаем новый материал без кромки, проверяем запись в БД, удаляем')
...
@testit.description('Создаем новый материал с 2-мя кромками, проверяем запись в БД, удаляем')
...
def test_post_greate_mat_not_existed_edge():
    ...
    with testit.step("проверка создания в описании нет КОДА404"):
        assert response.status_code == 422, 'материал возможно создан'
